  ...
  return (
    <div className="document-panel p-4">
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <p className="mono text-xs uppercase tracking-[0.12em] text-[#0B3D91]">
        ...
        {usableImages.map((image, imageIndex) => (
          <button
            ...
            className={`min-w-0 border p-2 ${
              imageIndex === index ? "border-[#0B3D91]" : "border-[#111111]/15"
            }`}
            aria-label={`Show image ${imageIndex + 1}`}
          >
            <span className="mono block text-xs uppercase tracking-[0.12em] text-[#0B3D91]">
